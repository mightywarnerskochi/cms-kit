                ...
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang->code }}-content" role="tabpanel">
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="translations[{{ $lang->code }}][country]" class="form-control @error("translations.{$lang->code}.country") is-invalid @enderror" value="{{ old("translations.{$lang->code}.country", $location->translations[$lang->code]['country'] ?? '') }}" placeholder="e.g. United Arab Emirates" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="translations[{{ $lang->code }}][title]" class="form-control @error("translations.{$lang->code}.title") is-invalid @enderror" value="{{ old("translations.{$lang->code}.title", $location->translations[$lang->code]['title'] ?? '') }}" placeholder="e.g. Deira, Dubai, UAE." required>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="translations[{{ $lang->code }}][address]" class="form-control @error("translations.{$lang->code}.address") is-invalid @enderror" rows="3" required>{{ old("translations.{$lang->code}.address", $location->translations[$lang->code]['address'] ?? '') }}</textarea>
                            ...
                            @foreach($extraFields as $key => $field)
                                <div class="col-md-6">
                                    ...
                                    @if(($field['type'] ?? 'text') === 'textarea')
                                        <textarea name="extra_fields[{{ $key }}]" class="form-control" rows="3">{{ old("extra_fields.{$key}", $location->extra_fields[$key] ?? '') }}</textarea>
                                    @elseif(($field['type'] ?? 'text') === 'select')
                                        <select name="extra_fields[{{ $key }}]" class="form-select">
                                            @foreach($field['options'] ?? [] as $optVal => $optLabel)
                                                <option value="{{ $optVal }}" {{ old("extra_fields.{$key}", $location->extra_fields[$key] ?? '') == $optVal ? 'selected' : '' }}>{{ $optLabel }}</option>
                                            @endforeach
                                        </select>
                                    @else
                                        <input type="text" name="extra_fields[{{ $key }}]" class="form-control" value="{{ old("extra_fields.{$key}", $location->extra_fields[$key] ?? '') }}">
                                    @endif
                                </div>
                            @endforeach
                        </div>
